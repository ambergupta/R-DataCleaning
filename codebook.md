...
A data.table named data_tidy_mean is set with the following columns. All units are maintained from the original data set. A file named data_tidy_mean.txt is written from run_analysis.R.
...
A data.table named data_tidy is set with the following columns. All units are maintained from the original data set. A file named data_tidy.txt is written from run_analysis.R.

Column	Original Name
Activity	
Subject	<From subject_test/train
...
Data description is here: http://archive.ics.uci.edu/ml/datasets/Human+Activity+Recognition+Using+Smartphones